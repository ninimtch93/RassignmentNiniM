...
MediaBucks %>% 
  select(Gender, Salary) %>% 
  group_by(Gender) %>% 
  summarize(Avg = mean(Salary, na.rm=TRUE))
...
```{r}
#View the range of your salary data   
summary (MediaSalary$Salary)
#Number of Rows
nrow (MediaSalary)
#Number of rows cut with filter of greater that 500 
nrow(MediaSalary %>% filter(MediaSalary$Salary > 5000))
...
NYT <- MediaBucks %>%
  filter (COMPANY == "NYT" | COMPANY == "NewYorkTimes") %>%
...
Bloomberg median salary of $50,000 is higher than the median $46,000 for general business reporter positions
...
```{r}
my_choice <- filter(MediaSalary, COMPANY %in% c("NewYorkTimes", "Politico", "CNN"))    
```
...
```{r}
#Who is making $230,504 as a reporter???   
