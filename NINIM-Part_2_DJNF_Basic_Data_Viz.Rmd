...
::: {style="text-align: center; color: #337DFF;"} \# Basic Visualization
in R
...
Using geom_bar() -- as opposed to geom_line() -- says we're making a bar
chart.
...
by total_loans. We can fix that by using the reorder() function to do
just that:
...
chart (also called a column chart) to a horizontal one. coord_flip()
does that for you.
...
kind of like [The Economist's](https://www.economist.com/) graphics?
...
We'll set the date_breaks to appear for every month; if we wanted every
week, we'd say date_breaks = "1 week". We can set the date to appear as
...
"axis.text.x = element_text(angle = 45, hjust=1)" we're saying, turn the
date labels 45 degrees.
